...
public class HoodieBloomIndex extends HoodieIndex<Object, Object> {
  ...
  public <R> HoodieData<HoodieRecord<R>> tagLocation(
    ...
    // Step 1: record -> (partitionPath, recordKey)
    ...
    if (LOG.isDebugEnabled()) {
      long totalTaggedRecords = keyFilenamePairs.count();//可以直接删掉,如果优化
  ...
  // 根据  集合(分区路径, recordkey) -> 集合(HoodieKey,HoodieRecordLocation)
  private HoodiePairData<HoodieKey, HoodieRecordLocation> lookupIndex(
    ...
    //拿到每个分区路径的布隆过滤器文件信息
    // fileInfoList 可以这么理解 <FileId ,BloomIndexFileInfo>, BloomIndexFileInfo中包含的key fields 的最大最小值
  ...
  //该方法返回  <FileId ,BloomIndexFileInfo >
  private List<Pair<String, BloomIndexFileInfo>> getBloomIndexFileInfoForPartitions(HoodieEngineContext context,
    ...
    //是否开启用keyfields range 剪枝
    if (config.getBloomIndexPruneByRanges()) {

      //hoodie.bloom.index.use.metadata
      //如果启用布隆索引的 metadata, 则从metadata中加载 keyfields ranges,否则从文件中重新计算该ranges,损失效率
      ...
      //与 loadColumnRangesFromFiles不同的是 BloomIndexFileInfo中的最大最小值没有赋值,不存在
  ...
  HoodieData<Pair<String, HoodieKey>> explodeRecordsWithFileComparisons(
    ...
    return partitionRecordKeyPairs.map(partitionRecordKeyPair -> {
      ...
      //getMatchingFilesAndPartition 方法返回的set中的元素是 Pair<分区路径,文件id>
      return indexFileFilter.getMatchingFilesAndPartition(partitionPath, recordKey).stream()
          .map(partitionFileIdPair -> (Pair<String, HoodieKey>) new ImmutablePair<>(partitionFileIdPair.getRight(),
              ...
              //list的元素的类型 Pair<文件id , HoodieKey<recordKey , 分区路径>>
  ...
  protected <R> HoodieData<HoodieRecord<R>> tagLocationBacktoRecords(
    ...
    // HoodiePairData<K, Pair<V, Option<W>>>
    return keyRecordPairs.leftOuterJoin(keyFilenamePair).values()
            // 右边有可能为null , 这种情况表明该条数据, 在现有的文件中找不到,是纯新增的
            //此时,HoodieRecord 已经被封装了地址 ,当然纯新增的数据地址为null
