...
public abstract class BaseFlinkCommitActionExecutor<T extends HoodieRecordPayload> extends
  ...
  // 本方法 是所有flink-hudi 桶索引普通插入数据 的一个 模版方法
  //  会根据 writeHandle 的类型 来路由到 handleInsert 和 handleUpdate 来处理
  //  但是 hudi 针对  mor 表 有其子类 重写的实现
  //  子类 BaseFlinkDeltaCommitActionExecutor 重写了 handleInsert 和 handleUpdate 方法

  //  不管子类还是 父类,  handleInsert 都会走生产消费模式  协调数据生产和消费速度不一致问题
  //

  //  handleUpdate 都会直接写入
  //  mor 走的核心流程 HoodieAppendHandle.doAppend()
  //  cow 走的核心流程 FlinkMergeHelper.newInstance().runMerge(table, upsertHandle)

  //  flink 使用 ExplicitWriteHandler 操作 FlinkAppendHandle、FlinkCreateHandle ; 使用 UpdateHandler 操作 FlinkMergeHandle 完成数据的写入
