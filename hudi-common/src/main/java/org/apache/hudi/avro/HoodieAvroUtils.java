...
public class HoodieAvroUtils {
  ...
  public static Schema addMetadataFields(Schema schema, boolean withOperationField) {
    ...
    //hoodie.allow.operation.metadata.field 开启才走 if，把_hoodie_operation 添加到 schema
    if (withOperationField) {
      final Schema.Field operationField =//_hoodie_operation
    ...
    if (fieldSchema.getType() == Schema.Type.UNION) {
      for (Schema schema : fieldSchema.getTypes()) {
        if (schema.getType() != Schema.Type.NULL) {
          //把字段中的 日期,decimal等特殊类型做转换 ,以适配 avro格式
