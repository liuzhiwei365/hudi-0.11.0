...
public class FlinkOptions extends HoodieConfig {
  ...
  //默认不同的 partition 的消息是不去重的,即相同的key消息,如果新消息换了partition,那么老的 partiiton 消息仍然保留
  //开启 index.global.enabled 选项开启跨partition 去重,原理是先往老的partiton发一条删除消息,再写新 partition

  //注意:   不同分区的 数据去重,  flinkhudi 的 桶索引目前做不到
