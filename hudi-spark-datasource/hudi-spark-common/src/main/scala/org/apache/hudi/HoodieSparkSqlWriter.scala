...
object HoodieSparkSqlWriter {
  ...
  : (Boolean, common.util.Option[String], common.util.Option[String], common.util.Option[String],
    ...
    // 定义 key 生成的方式 hoodie.datasource.write.keygenerator.class
      ...
      val (writeResult, writeClient: SparkRDDWriteClient[HoodieRecordPayload[Nothing]]) =
        operation match {
          ...
          case _ => {
            ...
            // 将sparksql 的 schema 转化为 avro 的 schema
            ...
            val hoodieRecords = {
              // hoodie.datasource.write.insert.drop.duplicates (只有insert操作类型不跟新历史数据 的情况下 , 才考虑开启这个功能,这个操作也会去查索引)
              // 将已经在存储系统中存在 的 数据丢弃掉 ( 用 record key 比较 )
              if (hoodieConfig.getBoolean(INSERT_DROP_DUPS)) {
  ...
  private def commitAndPerformPostOperations(spark: SparkSession,
    ...
    if (writeResult.getWriteStatuses.rdd.filter(ws => ws.hasErrors).count() == 0) {
      ...
      val metaMap = parameters.filter(kv =>
        kv._1.startsWith(parameters(COMMIT_METADATA_KEYPREFIX.key))) //hoodie.datasource.write.commitmeta.key.prefix

      // 提交事务
      // 内部会 调用 内联表服务(clean  archive  compact  cluster)
      val commitSuccess =
